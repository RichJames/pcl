...
;; *** Define a schema for our mp3 database ***
...
;; *** Make a table for holding MP3 information ***
...
;; *** Building a database from MP3 files on disk ***
...
(defun extract-schema (column-name schema)
  (loop :for c :in column-name :collect (find-column c schema)))
...
(defun random-selection (table n)
  (let* ((nbr-of-rows (length (rows table)))
	 (nbr-selected (if (< n nbr-of-rows) n nbr-of-rows)))
    (make-instance
     'table
     :schema (schema table)
     :rows (nshuffle-vector (random-sample (rows table) nbr-selected)))))
