...
(defclass playlist ()
  ((id           :accessor id           :initarg :id)
   ...
   (lock         :accessor lock         :initform (make-process-lock))))

(defun make-playlist-table ()
  (make-instance 'table :schema *mp3-schema*))

;;; *** macro to assist with process locking for multithreading ***
...
;; I think this method returns a *copy* of the playlist, not a pointer to the playlist.
...
;; We already have a current-song accessor method for playlist objects.  Howver,
;; we add this :around method to add thread locking to that accessor.
...
;; There is a generic function, (setf repeat) created by the defclass macro that created
;; playlist, that we can modify by defining an :after function, so we can do the additional
;; manipulation needed for the cases where the repeat value was changed to either :song or :all.
